...
class XMLTweakApplier {
	construct new() {
		 ...
		 *     "project/mytest/mytweak-1.xml",
		 *     "project/mytest/mytweak-2.xml",
		 *     "project/myothermod/tweak.xml",
		 *   },
		 * }
		 */
		_xml_tweaks = {}
	...
	handle_tweak_element(path, name, ext, elem, tweaks) {
		...
		if (control_key != null) {
			...
			var full_key = "project/%(path_parts[1])::%(control_key)"
...
class XMLLoader {
	static init() {
		...
		var blt_data_path = "project/saves/blt_data.txt"

		// Create a fiber to check if we are running in VR to prevent crashes with outdated dll versions
		(Fiber.new {
			var blt_data_path_vr = "project/saves/blt_data_vr.txt"
		...
		for (mod in IO.listDirectory("mods", true)) {
			...
			if (!disabled_mods[mod]) {
				...
				load_supermod_file("project/%(mod)", mod_data, false)
			}
		}
	...
	static handle_wren_tag(mod, tag, mod_data) {
		...
		if (tag["init-file"] != null) {
			var mod_name = mod.replace("project/", "")
